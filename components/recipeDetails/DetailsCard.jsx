import { getAllUser } from "@/db/queries";
...
export default async function DetailsCard({ recipeInfo }) {
    await dbConnect();
        ...
        <section>
            <div className="grid grid-cols-12 container gap-8 justify-items-center">
                ...
                <div className="col-span-12 md:col-span-6 py-8 flex flex-col justify-center">
                    <h2 className="font-semibold text-4xl lg:w-8/12 leading-10">{recipeInfo?.name}</h2>
                    <p className="text-xs text-[#eb4a36] italic my-2">{recipeInfo?.category}</p>
                    <p className="text-gray-600 text-sm my-6 leading-6">{recipeInfo?.description}</p>
                    ...
                    <div className="flex gap-4 justify-end">
                        <FavButton recipeInfo={recipeInfo} allUser={allUser} />

                        <ShareButton recipeInfo={recipeInfo} />
                    </div>
                </div>
            </div>
        </section>
